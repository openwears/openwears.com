...
class Page extends MarkdownPage
{
    ...
    public function __construct($page_name, $revision=null, $base_dir='../../wiki', $is_for_print=false, $lang='') {
    ...
    public function getPageFilePath() {
		$path = "{$this->base_dir}/{$this->lang}/{$this->page_name}." . self::extension;
		if(!file_exists($path)) {
			$path = "{$this->base_dir}/{$this->page_name}." . self::extension;
		}
    ...
    public static function getChapterList($base_dir = "../../wiki") {
        static $list = array();
	...
	public static function getFlatPagesArray($is_for_print=false) {
		...
		foreach(self::getFlatPages() as $page) {
			$pages[] = new self($page['page_name'], null, '../../wiki', $is_for_print);
		}
